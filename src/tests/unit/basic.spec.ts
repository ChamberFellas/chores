...
describe("To do list", () => {
    ...
    seedProducts = [
      {
        ...
        userID: [
          new Types.ObjectId("aaaabbbbccccddddeeeeffff"),
          new Types.ObjectId("111122223333444455556666"),
        ],
        houseID: "house1",
        description: "blah blah blah",
        deadline: new Date("2025-03-29"),
  ...
  test("Returns all chores where some chores incomplete", async () => {
    ...
    const response = await request(app).get(
      "/house1/chores/todo/aaaabbbbccccddddeeeeffff",
    );
    expect(response.status).toBe(200);
  });
  ...
  test("Returns nothing when no chores left", async () => {
    ...
    const response = await request(app).get(
      "/house1/chores/todo/111122223333444455556666",
    );
    expect(response.status).toBe(200);
  });
});
...
describe("completed chores", () => {
  ...
  test("Find specific chore Id", async () => {
    ...
    const response = await request(app).get(
      "/chores/completed/123456789abdef123456789",
    );
  ...
  test("Return nothing if id does not exist", async () => {
    jest
      .spyOn(Chore, "find")
      .mockResolvedValue(new Error("Database error") as any);
    jest.spyOn(app.response, "render").mockImplementation(() => {
      return app.response.end;
    });
    const response = await request(app).get(
      "/chores/completed/999988887777666655554444",
    );
    expect(response.status).toBe(200);
  });
});
...
describe("VerifiedCount", () => {
  ...
  test("Validcount increments on update. chore not yet complete", async () => {
    ...
    jest.spyOn(Chore, "findByIdAndUpdate");
    jest.spyOn(app.response, "render").mockImplementation(() => {
      return app.response.end;
    });
    jest
      .spyOn(axios, "post")
      .mockImplementation(async (url: string, item: any) => {
        if (url === "http://notifications-service:POST/notify") {
          return null; // just post notification, we don't rly care
        } else if (
          url === "http://users-service/get-all-users-in-house/house1"
        ) {
    ...
    const response = await request(app).put(
      "/aaaabbbbccccddddeeeeffff/house1/chores/completed/123456789abcdef123456789",
    );
